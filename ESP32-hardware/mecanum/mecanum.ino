...
const char *ssid = "wifiredian";
const char *passwd = "12345678";
const int port = 12345;

const IPAddress ip_address(192, 168, 4,200);
...
void loop() {
    ...
    if (message.length()) {
        ...
        if (begin != -1 && end != -1 && begin < end) {
            sscanf(message.substring(begin, end + 1).c_str(), "<%d,%d,%d,%d>",
                   &duty_cycle1, &duty_cycle2, &duty_cycle3, &duty_cycle4);
        }
    }
    message = "";
    xSemaphoreGive(mutex);
